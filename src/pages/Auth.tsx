...
export default function Auth() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (!regAgree) {
      toast({ variant: 'destructive', title: 'Ошибка', description: 'Необходимо принять условия использования' });
      return;
    }
  ...
  return (
    <>
      <PageSEO title="Авторизация" description="Войдите или зарегистрируйтесь" path="/auth" />
      <div className="flex min-h-[calc(100vh-3.5rem)] items-center justify-center px-4 py-8">
        <Card className="w-full max-w-md mx-auto">
          <CardHeader className="text-center">
            ...
            <CardTitle className="text-2xl font-bold">Право БУ</CardTitle>
            <CardDescription>Ваш правовой помощник</CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <TabsContent value="login" className="space-y-4 pt-4">
                <form onSubmit={handleLogin} className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Input id="login-email" type="email" value={loginEmail} onChange={(e) => setLoginEmail(e.target.value)} required placeholder="user@example.com" />
                  </div>
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <Label htmlFor="login-password">Пароль</Label>
                      <Link to="/auth/reset-password" className="text-xs text-primary hover:underline">Забыли пароль?</Link>
                    </div>
                  ...
                  <>
                    <form onSubmit={handleRegister} className="space-y-4">
                      <div className="space-y-2">
                        ...
                        <Input id="reg-name" type="text" value={regName} onChange={(e) => setRegName(e.target.value)} placeholder="Иван Иванов" />
                      </div>
                      <div className="space-y-2">
                        <Label htmlFor="reg-email">Электронная почта</Label>
                        <Input id="reg-email" type="email" value={regEmail} onChange={(e) => setRegEmail(e.target.value)} required placeholder="user@example.com" />
                      </div>
                      <div className="space-y-2">
                        <Label htmlFor="reg-password">Пароль</Label>
                        <Input id="reg-password" type="password" value={regPassword} onChange={(e) => setRegPassword(e.target.value)} required minLength={8} placeholder="Минимум 8 символов" />
                      </div>
                      <div className="space-y-2">
                        <Label htmlFor="reg-confirm">Подтверждение пароля</Label>
                        <Input id="reg-confirm" type="password" value={regConfirm} onChange={(e) => setRegConfirm(e.target.value)} required minLength={8} placeholder="Повторите пароль" />
                      </div>
                      <div className="flex items-start gap-2">
                        <Checkbox id="reg-agree" checked={regAgree} onCheckedChange={(v) => setRegAgree(v === true)} className="mt-0.5" />
                        <label htmlFor="reg-agree" className="text-sm text-muted-foreground leading-tight cursor-pointer">
                          Я соглашаюсь с{' '}
                          <Link to="/legal" className="text-primary underline hover:text-primary/80">условиями использования</Link>
                        </label>
                      </div>
