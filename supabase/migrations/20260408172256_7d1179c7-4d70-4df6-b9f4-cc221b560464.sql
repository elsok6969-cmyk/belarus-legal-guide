...
DROP INDEX IF EXISTS idx_doc_sections_fts;

-- Create indexes matching search_all expressions
CREATE INDEX idx_documents_fts ON documents USING GIN (
  to_tsvector('russian', coalesce(title,'') || ' ' || coalesce(short_title,'') || ' ' || coalesce(content_text,''))
);

CREATE INDEX idx_sections_fts ON document_sections USING GIN (
  to_tsvector('russian', coalesce(title,'') || ' ' || coalesce(content_text,''))
);

CREATE INDEX IF NOT EXISTS idx_sections_number ON document_sections (number);

-- Recreate search_all with ILIKE fallback
...
BEGIN
  ...
  IF article_num IS NOT NULL THEN
    ...
    WHERE (ds.number = article_num OR ds.number = 'Статья ' || article_num)
